...
class DocumentDeletionWorkerTest {
    ...
    @Autowired DocumentDeletionWorker worker;
    @Autowired DocumentEscalationService escalationService;
    ...
    void setUp() {
        ...
        KycSubmission submission = submissionRepository.save(
                new KycSubmission(UUID.randomUUID(), "GHA-123456789-0", "Test", "corr-1"));

        document = new KycSubmissionDocument(submission.getId(), "FRONT_OF_CARD", "LOCAL",
                "submissions/abc/front.jpg", "image/jpeg", 1024L, "hash1");
    ...
    void happy_deletion() {
        ...
        KycSubmissionDocument reloaded = documentRepository.findById(document.getId()).orElseThrow();
    ...
    @Test
    @DisplayName("storage failure: status=DELETE_FAILED, failure_count incremented, audit row inserted")
    void storage_failure_increments_count() {
        ...
        assertThat(reloaded.getStorageKey()).isEqualTo("submissions/abc/front.jpg");
    ...
    @Test
    @DisplayName("partial failure: one document fails, others in same batch still attempted")
    void partial_failure_does_not_stop_others() {
        KycSubmission sub = submissionRepository.findAll().get(0);

        KycSubmissionDocument doc2 = documentRepository.save(new KycSubmissionDocument(
                sub.getId(), "BACK_OF_CARD", "LOCAL", "submissions/abc/back.jpg", "image/jpeg", 512L, "h2"));
        ...
        doThrow(new RuntimeException("timeout")).when(objectStorage).delete("submissions/abc/front.jpg");
        doNothing().when(objectStorage).delete("submissions/abc/back.jpg");

        worker.attemptDeletion(document);
        worker.attemptDeletion(doc2);

        KycSubmissionDocument reloaded1 = documentRepository.findById(document.getId()).orElseThrow();
        KycSubmissionDocument reloaded2 = documentRepository.findById(doc2.getId()).orElseThrow();
    ...
    @Test
    @DisplayName("max attempts reached: document marked DELETE_FAILED, escalation triggered")
    void max_attempts_triggers_escalation() {
        doThrow(new RuntimeException("persistent failure")).when(objectStorage).delete(any());
    ...
    @Test
    @DisplayName("already-DELETED document: scheduler does not select it (no PENDING_DELETION)")
    void already_deleted_not_selected() {
        document.setDeletionStatus("DELETED");
        document.setStorageKey(null);
        document = documentRepository.save(document);

        List<KycSubmissionDocument> due = documentRepository.findDueForDeletion(100);
    ...
    @Test
    @Transactional
    @DisplayName("scheduler: SELECT FOR UPDATE SKIP LOCKED query selects due documents")

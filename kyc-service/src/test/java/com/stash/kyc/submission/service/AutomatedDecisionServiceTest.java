...
class AutomatedDecisionServiceTest {
    ...
    static void configure(DynamicPropertyRegistry registry) {
        ...
        registry.add(
            "stash.kyc.stub-provider.test-approve-numbers",
            () -> "GHA-111111111-1"
        );
    }

    @Autowired
    AutomatedDecisionService decisionService;
    ...
    @Test
    @DisplayName(
        "happy approval: test card number transitions submission to APPROVED"
    )
    void happy_approval() {
    ...
    void approval_records_decision() {
        ...
        assertThat(
            decisionRecordRepository.findAll().get(0).getDecision()
        ).isEqualTo("PASS");
    }

    @Test
    @DisplayName(
        "approval does not schedule document deletion inline — event consumer handles it"
    )
    void approval_does_not_schedule_deletion_inline() {
        ...
        documentRepository.save(doc);
        decisionService.processSubmission(submission.getId(), "corr-1");
        var reloadedDoc = documentRepository
            .findById(doc.getId())
            .orElseThrow();
    ...
    @Test
    @DisplayName(
        "non-test card number routes submission to manual review without terminal rejection"
    )
    void non_test_number_routes_to_manual_review() {
        ...
        assertThat(reloaded.getDecision()).isNull();
    ...
    void rejection_creates_queue_entry() {
        ...
        assertThat(
            manualReviewQueueRepository.findAll().get(0).getSubmissionId()
        ).isEqualTo(submission.getId());
    }

    @Test
    @DisplayName("manual escalation does not publish kyc.rejected event")
    void rejection_does_not_publish_terminal_event() {
        KycSubmission submission = submissionInReviewing("GHA-999999999-9");
        decisionService.processSubmission(submission.getId(), "corr-1");
    ...
    @Test
    @DisplayName(
        "transient provider failure routes to manual review after retries exhausted"
    )
    void transient_failure_routes_to_manual() {
        GhanaCardProviderClient failingProvider = mock(
            GhanaCardProviderClient.class
        );
        when(failingProvider.verify(any(), any(), any())).thenThrow(
            new RuntimeException("Simulated transient provider failure")
        );
        AutomatedDecisionService serviceWithFailingProvider =
            new AutomatedDecisionService(
                submissionRepository,
                decisionRecordRepository,
                manualReviewQueueRepository,
                failingProvider,
                rabbitTemplate
            );
        KycSubmission submission = submissionInReviewing("GHA-111111111-1");
        serviceWithFailingProvider.processSubmission(
            submission.getId(),
            "corr-1"
        );

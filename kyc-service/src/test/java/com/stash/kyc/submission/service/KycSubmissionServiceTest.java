...
class KycSubmissionServiceTest {
    ...
    void happy_submission_returns_upload_urls() {
        ...
        assertThat(response.uploadUrls()).containsKeys(
    ...
    @Test
    @DisplayName(
        "ghana_card_number is stored as ciphertext, not plaintext, in raw column value"
    )
    ...
    @Test
    @DisplayName(
        "second submission while one is active returns 409 KYC_SUBMISSION_ALREADY_ACTIVE"
    )
    ...
    @Test
    @DisplayName(
        "a different user can submit while another user has an active submission"
    )
    void different_user_not_blocked() {
        ...
        assertThatNoException().isThrownBy(() ->
            kycSubmissionService.createSubmission(otherUser, req())
        );
    }
}

...
class KycSubmissionStatusServiceTest {
    ...
    @Autowired KycSubmissionStatusService statusService;
    ...
    private KycSubmission save(UUID userId, String status) {
        KycSubmission s = new KycSubmission(userId, "GHA-123456789-0", "Test User", "corr-1");
        s.setStatus(status);
        if (KycSubmission.STATUS_APPROVED.equals(status) || KycSubmission.STATUS_REJECTED.equals(status)) {
    ...
    @Test
    @DisplayName("REVIEWING status returned correctly (UNDER_REVIEW per issue, REVIEWING per schema)")
    ...
    void rejected_status_includes_reason() {
        ...
        assertThat(response.rejectionReason()).isEqualTo("Card image unreadable");
    }
    ...
    void unknown_submission_returns_404() {
        assertThatExceptionOfType(StashApiException.class)
                .isThrownBy(() -> statusService.getStatus(UUID.randomUUID(), USER_A))
                .satisfies(ex -> assertThat(ex.getHttpStatus()).isEqualTo(HttpStatus.NOT_FOUND));
    }
    ...
    @Test
    @DisplayName("getMyStatus returns the most recent submission when multiple exist")
    void me_returns_most_recent() {
        save(USER_A, KycSubmission.STATUS_REJECTED);
        KycSubmission newest = save(USER_A, KycSubmission.STATUS_PENDING_DOCUMENTS);
    ...
    @Test
    @DisplayName("response contains no ghana_card_number or storage-related fields")

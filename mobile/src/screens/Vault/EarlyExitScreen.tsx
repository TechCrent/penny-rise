...
type RouteParams = { EarlyExit: { vaultId: string } };
...
export default function EarlyExitScreen() {
  ...
  const route       = useRoute<RouteProp<RouteParams, 'EarlyExit'>>();
  const { vaultId } = route.params;

  const { data: vault } = useVaultDetail(vaultId);
  const { mutateAsync: requestExit, isPending: requesting } =
    useRequestEarlyExit(vaultId);
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.explainerCard}>
            ...
            <Text style={styles.explainerBody}>
              Life happens. You can exit this vault before your conditions are
              met. A{' '}
              <Text style={styles.explainerBold}>5% fee</Text> applies to your
              balance, and there's a{' '}
              <Text style={styles.explainerBold}>72-hour wait</Text> before the
              money is released — giving you time to change your mind.
            </Text>
          </View>
          ...
          <Text style={styles.sectionSub}>
            This is collected for product research only — it doesn't change
            your fee or the processing time.
          </Text>
          ...
          {REASON_OPTIONS.map(opt => (
            <TouchableOpacity
              ...
              style={[
                styles.reasonCard,
                reason === opt.id && styles.reasonCardSelected,
              ]}
            ...
            >
              ...
              <View style={styles.reasonBody}>
                <Text style={[
                  styles.reasonLabel,
                  reason === opt.id && styles.reasonLabelSelected,
                ]}>
                  {opt.label}
                </Text>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.breakdownCard}>
            <BreakdownRow
              ...
              value={`GHS ${formatCedis(balancePesewas)}`}
            />
            <BreakdownRow
          ...
          <View style={styles.coolOffCard}>
            <Text style={styles.coolOffTitle}>⏳{'  '}<Text>72-hour cool-off</Text></Text>
            <Text style={styles.coolOffBody}>
              Your money won't be released immediately. You can cancel at any
              time before{' '}
          ...
          <View style={styles.depositNote}>
            <Text style={styles.depositNoteText}>
              💡 Any deposits you make <Text style={styles.depositNoteBold}>during the 72-hour window</Text>{' '}
              will be included in your release — the 5% fee only applies to your
              balance right now.
            </Text>
          </View>
          ...
          >
            <Text style={styles.cancelLinkText}>
              Keep my lock — I've changed my mind
            </Text>
          </TouchableOpacity>
        </ScrollView>
      </SafeAreaView>
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.confirmIntro}>
            ...
            <Text style={styles.confirmIntroTitle}>
              You're starting the exit process
            </Text>
            <Text style={styles.confirmIntroBody}>
              Once confirmed, the 72-hour cool-off begins. Your vault will show
              as <Text style={styles.confirmIntroBold}>Early Exit Pending</Text>.
              You can cancel at any time before the 72 hours are up.
            </Text>
          </View>
          ...
          >
            {requesting ? (
              ...
              <Text style={styles.ctaText}>
                I understand — start the 72-hour cool-off
              </Text>
            )}
          </TouchableOpacity>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        ...
        >
          ...
          {/* Release summary */}
          <View style={[styles.breakdownCard, { width: '100%' }]}>
            <BreakdownRow
          ...
          <View style={styles.doneNote}>
            <Text style={styles.doneNoteText}>
              Your vault now shows as{' '}
              <Text style={styles.doneNoteBold}>Early Exit Pending</Text>. You
              can still cancel from the vault screen before the deadline — no
              fee will be charged if you cancel.
            </Text>
          </View>
...
export function EarlyExitStatusPanel({
  ...
  const handleCancel = useCallback(async () => {
    Alert.alert(
      'Cancel early exit?',
      'Your vault will return to ACTIVE. No fee will be charged.',
      [
    ...
    <View style={panelStyles.container}>
      ...
      <View style={panelStyles.headerRow}>
        ...
        <View style={panelStyles.headerBody}>
          ...
          <Text style={panelStyles.headerCountdown}>
            {formatCountdown(scheduledReleaseAt)}
          </Text>
        </View>
      </View>
      ...
      <Text style={panelStyles.releaseDate}>
        Releases on {formatDatetime(scheduledReleaseAt)}
      </Text>

      {cancelError && (
        <Text style={panelStyles.cancelError}>{cancelError}</Text>
      )}
      ...
      <Text style={panelStyles.cancelNote}>
        Cancel before the deadline — no fee applies.
      </Text>
    </View>
  );
}
...
function BreakdownRow({
  ...
  negative?: boolean; positive?: boolean; bold?: boolean;
}) {
  return (
...
function FinalRow({ label, value, negative, bold }: {
...
const styles = StyleSheet.create({
  ...
  explainerTitle: { fontSize: 16, fontWeight: '700', color: DARK, marginBottom: 6, textAlign: 'center' },
  ...
  reasonCard: {
    ...
    marginBottom: 10, borderWidth: 2, borderColor: '#EDEDF0',
  },
  ...
  radioOuter: {
    width: 22, height: 22, borderRadius: 11, borderWidth: 2, borderColor: INDIGO,
  ...
  releaseLabel:     { fontSize: 13, color: MUTED, fontWeight: '600', textTransform: 'uppercase',
  ...
  breakdownCard: {
    ...
    marginBottom: 16, borderWidth: 1, borderColor: '#EDEDF0', overflow: 'hidden',
  },
  ...
  confirmIntroTitle: { fontSize: 16, fontWeight: '700', color: DARK, marginBottom: 6, textAlign: 'center' },
  ...
  doneCountdown:    { fontSize: 28, fontWeight: '800', color: INDIGO, letterSpacing: -0.8, marginBottom: 24 },
  doneRelease:      { fontSize: 13, color: MUTED, textAlign: 'center', marginVertical: 16 },
  doneReleaseBold:  { fontWeight: '700', color: DARK },
  doneNote:         { backgroundColor: '#F0FDF4', borderRadius: 12, padding: 14,
  ...
  // Shared CTA
  cta: {
    backgroundColor: INDIGO, borderRadius: 14, paddingVertical: 15, alignItems: 'center',
  ...
  ctaText:     { fontSize: 15, fontWeight: '700', color: '#FFFFFF', textAlign: 'center', lineHeight: 20 },
});
...
const panelStyles = StyleSheet.create({
  container: {
    ...
    margin: 16, borderWidth: 1, borderColor: '#FDE68A',
  },
  ...
  amountRow: {
    ...
    backgroundColor: 'rgba(255,255,255,0.6)', borderRadius: 12, padding: 14, marginBottom: 10,
  },
  amountItem: { flex: 1, alignItems: 'center' },
  amountDivider: { width: 1, height: 36, backgroundColor: '#FDE68A' },
  amountLabel: { fontSize: 11, color: '#B45309', fontWeight: '600',
  ...
  cancelBtn: {
    borderWidth: 1.5, borderColor: RED, borderRadius: 12, paddingVertical: 13,
    alignItems: 'center', backgroundColor: '#FEF2F2', marginBottom: 8,
  },

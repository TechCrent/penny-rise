  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      if (code === 'VAULT_INSUFFICIENT_BALANCE') {
        ...
        setServerError(
          'LOCKED vaults cannot withdraw via this screen. ' +
          'Use the early-exit flow instead.',
        );
      } else if (status === 403) {
          ...
          >
              ...
              {PROVIDERS.map(p => (
                <TouchableOpacity
                  ...
                  style={[styles.providerPill,
                          provider === p.id && PROVIDER_PILL_ACTIVE[p.id]]}
                ...
                >
                  <Text style={[styles.providerText,
                                provider === p.id && PROVIDER_TEXT_COLOR[p.id]]}>
                    {p.label}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <View style={styles.warningBox}>
              ...
              <Text style={styles.warningText}>
                ...
                <Text style={styles.warningBold}>5–10 minutes</Text>. Occasionally
                up to 24 hours during network congestion.
              </Text>
            </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => { if (validateAmount()) setPhase('confirm'); }}
              activeOpacity={0.85}
          ...
          <View style={styles.settlementNote}>
            ...
            <View style={styles.settlementBody}>
              ...
              <Text style={styles.settlementDesc}>
                Your money will arrive on {momoNumber} within 5–10 minutes.
      ...
      >
        ...
        {txnRef && (
          <View style={styles.refCard}>
            ...
            <Text style={styles.refValue} selectable>{txnRef}</Text>
          </View>
        )}
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoBody}>
            Paystack is sending the money to your MoMo number. This usually
            takes{' '}
            <Text style={styles.infoBold}>5–10 minutes</Text>. You&apos;ll receive
            a push notification when it lands. You don&apos;t need to keep this
            screen open.
          </Text>
        </View>
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoBody}>
            Check your vault statement — the transaction will show as PENDING.
            If it stays PENDING for more than 24 hours, contact support with
...
const styles = StyleSheet.create({
  ...
  // Amount phase
  balanceCard: {
    backgroundColor: DARK, borderRadius: 18, padding: 20, marginBottom: 24,
    shadowColor: DARK, shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.14, shadowRadius: 12, elevation: 4,
  },
  balanceLabel: { fontSize: 11, color: 'rgba(255,255,255,0.55)', fontWeight: '600',
  ...
  fieldLabel: { fontSize: 12, fontWeight: '700', color: DARK, marginBottom: 8,
                textTransform: 'uppercase', letterSpacing: 0.4 },
  fieldLabelDest: { fontSize: 12, fontWeight: '700', color: DARK, marginBottom: 8,
                    textTransform: 'uppercase', letterSpacing: 0.4, marginTop: 24 },

  amountRow:        { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 },
  ghsPrefix:        { fontSize: 20, fontWeight: '600', color: MUTED },
  amountInput:      { flex: 1, fontSize: 40, fontWeight: '800', color: DARK, letterSpacing: -1.5,
                      paddingVertical: 4, borderBottomWidth: 2, borderBottomColor: '#D1D5DB' },
  amountInputError: { borderBottomColor: '#EF4444' },
  afterBalance:     { fontSize: 13, color: GREEN, fontWeight: '600', marginBottom: 16 },

  exceedsBox:  { backgroundColor: '#FEF2F2', borderRadius: 10, padding: 12,
  ...
  warningBox:  { flexDirection: 'row', alignItems: 'flex-start', gap: 8,
  ...
  // Confirm phase
  summaryCard: {
    backgroundColor: DARK, borderRadius: 20, padding: 24, marginBottom: 16,
  ...
  settlementNote:  { flexDirection: 'row', alignItems: 'flex-start', gap: 12,
  ...
  serverErrorBox:  { backgroundColor: '#FEF2F2', borderRadius: 12, padding: 14,
                     marginBottom: 12, borderWidth: 1, borderColor: '#FCA5A5' },
  ...
  pendingAmount:      { fontSize: 36, fontWeight: '800', color: INDIGO,
                        letterSpacing: -1.2, marginBottom: 6 },
  pendingDestination: { fontSize: 14, color: MUTED, fontWeight: '500' },

  pendingStatusCard: { backgroundColor: '#FFFFFF', borderRadius: 16, padding: 20,
                       marginBottom: 16, borderWidth: 1, borderColor: '#EDEDF0' },
  ...
  statusConnector:   { width: 2, height: 18, backgroundColor: '#E5E7EB',
                       marginLeft: 5, marginVertical: 4 },

  refCard:  { backgroundColor: '#F3F4F6', borderRadius: 12, padding: 14,
              marginBottom: 16, alignItems: 'center' },
  refLabel: { fontSize: 11, color: MUTED, fontWeight: '600', textTransform: 'uppercase',
              letterSpacing: 0.4, marginBottom: 4 },
  refValue: { fontSize: 13,
              fontFamily: Platform.OS === 'ios' ? 'Courier New' : 'monospace',
              color: DARK, fontWeight: '600' },

  infoBox:   { backgroundColor: '#FFFFFF', borderRadius: 14, padding: 16,
               marginBottom: 12, borderWidth: 1, borderColor: '#EDEDF0' },

...
class VaultCreationServiceTest {
    ...
    void setUp() {
        ...
        when(userRepo.findByIdForVaultCreation(USER_ID)).thenReturn(
            Optional.of(approvedFreeUser())
        );
        when(
            vaultRepo.countByOwnerUserIdAndVaultType(USER_ID, "STANDARD")
        ).thenReturn(0L);
        when(
            vaultRepo.countByOwnerUserIdAndVaultType(USER_ID, "LOCKED")
        ).thenReturn(0L);
        when(
            paymentsClient.provisionVaultLedgerAccount(
                any(),
                any(),
                any(),
                any()
    ...
    void standard_vault_created() {
        var result = service.createVault(
            USER_ID,
            standardRequest(),
            CORR_ID,
            IDEM_KEY
    ...
    @Test
    @DisplayName("same idempotency key reuses the same deterministic vault id")
    void same_idempotency_key_reuses_same_vault_id() {
        service.createVault(USER_ID, standardRequest(), CORR_ID, IDEM_KEY);
        service.createVault(USER_ID, standardRequest(), CORR_ID, IDEM_KEY);

        verify(paymentsClient, times(2)).provisionVaultLedgerAccount(
            eq(USER_ID),
            eq(
                UUID.nameUUIDFromBytes(
                    (USER_ID + ":" + IDEM_KEY).getBytes(
    ...
    void locked_vault_date_only() {
        var req = new CreateVaultRequest(
            "University Fund",
            "LOCKED",
            Instant.parse("2028-06-24T00:00:00Z"),
            null,
            null
        );
    ...
    @Test
    @DisplayName(
        "LOCKED vault with amount only: unlock_condition_logic is null"
    )
    ...
    @Test
    @DisplayName(
        "LOCKED vault with both conditions: unlock_condition_logic defaults to AND"
    )
    ...
    void locked_no_conditions_returns_422() {
        ...
        assertThatThrownBy(() ->
            service.createVault(USER_ID, req, CORR_ID, IDEM_KEY)
        )
    ...
    void premium_user_no_limit() {
        when(userRepo.findByIdForVaultCreation(USER_ID)).thenReturn(
            Optional.of(premiumApprovedUser())
        );
        when(
            vaultRepo.countByOwnerUserIdAndVaultType(USER_ID, "STANDARD")
    ...
    @Test
    @DisplayName(
        "Payments Service failure: returns 502, vault row NOT persisted"
    )
    void payments_failure_vault_not_persisted() {
        when(
            ...
            new PaymentsServiceException("timeout", 503, new RuntimeException())
        );
    ...
    @Test
    @DisplayName(
        "concurrent creation: second request sees updated count after first commits"
    )
    void concurrent_creation_second_sees_limit() {
        ...
        // First succeeds
        assertThatCode(() ->
            service.createVault(USER_ID, standardRequest(), CORR_ID, "idem-001")
        ).doesNotThrowAnyException();
